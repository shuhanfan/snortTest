...
//	 }
	 ...
	 public static boolean IndexKMP(String ori, String par, int nk,Map<Integer,ArrayList<Integer> > res,boolean isStr,boolean ignoreCase, boolean negative) {
		...
		while(i<ori.length()){
			...
			if(j==-1||ori.charAt(i)==par.charAt(j) || ignoreCase &&String.valueOf(par.charAt(j)).toUpperCase().equals(String.valueOf(par.charAt(j)).toUpperCase())) {
				i++;
				j++;
			}
			else j=next[j];	
	 ...
	 public static boolean KMP(String value,byte[] payload,int nk,Map<Integer,ArrayList<Integer> > res, boolean ignoreCase) {//value是匹配的原串
	try{
		//fw.write("in KMP\n");
		...
		// System.out.println("the oris is:"+oris);
		 ...
		 if(pat.length==1){//只有字符串匹配
			 ...
			 isString = true;
			 //return true;
			 return IndexKMP(oris,pat[0],nk,res,isString,ignoreCase,negative);
		 }
		 else if(pat.length==2&&("").equals(pat[0])){//只有二进制匹配
			 System.out.println("pat2");
			// return true;
			 pat[1].replace(" ", "");
			 return IndexKMP(orib,pat[1],nk,res,isString,ignoreCase,negative);
				 ...
				 for(int i=0; i<re.size(); ){
					 ...
					 for(; j<pat.length; j++){
						 ...
						 if(j%2==1){//match byte
							 ...
							 if(newstart*2+p.length()>oris.length()){
								 System.out.println("newstart*2+p.length()>oris.length()");
								 re.remove(i);
								 if(re.size() == 0){
									 System.out.println("re.size() == 0");
									 return false;
					 ...
					 if(j == pat.length) {//else because of break and remove a offset,so not need i++ when check the next match
		 ...
		 try{
			...
			for(int k=0; k<ro.conMap.size(); k++){
				...
				//for(Iterator<Integer> it = mat.iterator(); it.hasNext();){
					...
					if(pos>offset&&pos+pattern.length()-1<=depth+offset) {
					//if(h>offset&&h+pattern.length()-1<=depth+offset){
					  ...
					  //检测该content匹配位置是否满足distance
						...
						if(within!=0 && k>0){
							...
							int lastmat_size = lastmat.size();
							for(int j=0; j<lastmat_size; j++){
								if(lastmat.get(j)+distance+within>=pos+pattern.length()-1){
									isOk=true;	
									break;
								}
						...
						if(tmp.containsKey(" pkt_data")) {
							...
							mat.set(h, pos+pattern.length()-1);
						}
						//将匹配起始值更新为匹配终止值
						h++;
